...
`shared: true` (the iPhone share sheet) keeps the text in maybe when the detector would
exclude it, and so does a message the person forwards to their Witness address themself.
None of these keeps violence, threats, self-harm or goodbyes: those are always excluded. Images up to 10 MB (JPEG, PNG, WebP, HEIC, GIF; the file's
